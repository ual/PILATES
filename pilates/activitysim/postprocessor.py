...
def copy_output_plans_to_beam(settings):
    asim_output_data_dir = settings['asim_local_output_folder']
    plans_path = os.path.join(asim_output_data_dir, 'final_plans.csv')
    beam_local_plans = os.path.join(settings['beam_local_input_folder'], settings['beam_plans'])
...
# def cleanup():
#     """
#     Method to remove/archive data from the input directory that was
#     created on-the-fly during simulation. Without removing these files
#     after we're done with them, ActivitySim would simply re-use them
#     instead of updating them with new skims/persons/etc generated by the
#     other simulations
#     """

#     # these should all be preserved in archived urbansim output data
#     # delete skims
#     # delete households, persons, land_use csv's
